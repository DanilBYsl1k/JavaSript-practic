class assortiment{
    ...
    createAssortiment(){
        ...
        document.querySelector('.assortiment').appendChild(ul)
        ul.classList.add('catalog-list')
        for (const key in this.item){
            let li=document.createElement('li');
            document.querySelector('.catalog-list').appendChild(li);
            li.classList.add(`catalog-item-list`)
            li.classList.add(this.item[key].class)
            li.setAttribute('data-price',this.item[key].price)
            li.setAttribute('data-rating',this.item[key].rating)
            li.setAttribute('data-id',this.item[key].id)
            li.innerHTML=
            `<div class="block-item-top"><a href="blank.html">
                <img data-info="vl" class="product-img" src="${this.item[key].img}">
            </a></div>
             <div class="price-name">
                    <h3 class="item-title">${this.item[key].name}</h3>
                    <p>${this.item[key].Availability}</p>
                <div class="price-shopping-car">
                    <div class="price">
                    <p data-price>${this.item[key].price}</p>
                        <div class="amount">
                            <p data-amount>${this.item[key].amount} шт</p> 
                        </div>
                    </div>
                    <div class="shopping-car" data-buy="add">
...
const assortiments=new assortiment()

assortiments.createAssortiment()